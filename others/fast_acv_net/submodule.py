...
def groupwise_correlation_norm(fea1, fea2, num_groups):
    ...
    cost = (
        (fea1 / (torch.norm(fea1, 2, 2, True) + 1e-05))
        * (fea2 / (torch.norm(fea2, 2, 2, True) + 1e-05))
    ).mean(dim=2)
    assert cost.shape == (B, num_groups, H, W)
    return cost
...
def build_gwc_volume_norm(refimg_fea, targetimg_fea, maxdisp, num_groups):
    ...
    for i in range(maxdisp):
        if i > 0:
            ...
            volume[:, :, i, :, :] = groupwise_correlation_norm(
                refimg_fea, targetimg_fea, num_groups
            )
    volume = volume.contiguous()
    return volume
...
class Propagation(nn.Module):
    ...
    def forward(self, disparity_samples):
        one_hot_filter = np.zeros(shape=(5, 1, 3, 3), dtype=np.float32)
        ...
        one_hot_filter = torch.tensor(
            one_hot_filter, dtype=torch.float32, device=disparity_samples.device
        )
        disparity_samples = self.replicationpad(disparity_samples)
        aggregated_disparity_samples = F.conv2d(
            disparity_samples, one_hot_filter, padding=0
        )
...
class Propagation_prob(nn.Module):
    ...
    def forward(self, prob_volume):
        one_hot_filter = np.zeros(shape=(5, 1, 1, 3, 3), dtype=np.float32)
        ...
        one_hot_filter = torch.tensor(
            one_hot_filter, dtype=torch.float32, device=prob_volume.device
        )
