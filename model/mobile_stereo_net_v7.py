...
class CostAggregateHourglass3D(nn.Module):
    def __init__(self, in_dim, fmap_dims, use_conv_final):
        super(CostAggregateHourglass3D, self).__init__()

        self.in_dim = in_dim
        # fmap_dims = [fmap_dim_8x, fmap_dim_16x]
        ...
        self.conv1_up = BasicConv(
            in_dim * 2,
            in_dim if use_conv_final else 1,
            deconv=True,
            is_3d=True,
            bn=False,
            relu=False,
        ...
        self.feature_att_2x = channelAtt(in_dim * 2, fmap_dims[0])
        self.feature_att_4x = channelAtt(in_dim * 4, fmap_dims[1])
        self.feature_att_up_2x = channelAtt(in_dim * 2, fmap_dims[0])

        self.conv_final = (
            nn.Conv3d(in_dim, 1, 3, 1, 1, bias=False) if use_conv_final else None
        )

    def forward(self, x, fmaps):
        conv1 = self.conv1(x)
        conv1 = self.feature_att_2x(conv1, fmaps[0])

        conv2 = self.conv2(conv1)
        conv2 = self.feature_att_4x(conv2, fmaps[1])

        conv2_up = self.conv2_up(conv2)
        conv1 = torch.cat((conv2_up, conv1), dim=1)
        conv1 = self.aggregate(conv1)
        conv1 = self.feature_att_up_2x(conv1, fmaps[0])
    ...
    def forward(self, left, right, is_train=True):
        ...
        if self.use_concat_volume:
            ...
            seman_weights_4x = self.concat_aggregate(
                concat_volume, features_left[1:3]
            ).squeeze(1)
