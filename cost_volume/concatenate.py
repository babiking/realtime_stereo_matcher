...
class TorchConcatenateCost(nn.Module):
    ...
    def forward(self, left, right):
        ...
        Return:
            [1] volume, (N, 2 * C, max_disparity, H, W), cost volume
                - volume[:, :C, d, y, x], left feature vector at pixel (x, y) aligned with disparity level d
                - volume[:, :C, d, y, x], right feature vector at pixel (x, y) aligned with disparity level d
        """
        n, c, h, w = left.shape

        volume = torch.zeros(
            size=(n, 2 * c, self.max_disparity, h, w),
            dtype=left.dtype,
            device=left.device,
        )
